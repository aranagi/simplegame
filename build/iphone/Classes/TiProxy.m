...
//Rare exceptions to indicate a bug in the _simplegame code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_simplegame = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_simplegame]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.simplegame.module"
